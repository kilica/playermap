...
require_once PLAYERMAP_TRUST_PATH . '/admin/class/installer/PlayermapInstallUtils.class.php';

/**
 * Playermap_Uninstaller
**/
class Playermap_Uninstaller
{
    ...
    **/
    private function _uninstallModule()
    {
        ...
        {
            $this->mLog->addReport(_MI_PLAYERMAP_INSTALL_MSG_MODULE_INFORMATION_DELETED);
        }
        else
        {
            $this->mLog->addError(_MI_PLAYERMAP_INSTALL_ERROR_MODULE_INFORMATION_DELETED);
        }
    }
        ...
        if(is_array($tables))
        {
            foreach($tables as $table)
            {
                ...
                if($db->query($sql))
                {
                    $this->mLog->addReport(
                        XCube_Utils::formatString(
                            _MI_PLAYERMAP_INSTALL_MSG_TABLE_DOROPPED,
                            $tableName
                        )
                    );
                }
                else
                {
                    $this->mLog->addError(
                        XCube_Utils::formatString(
                            _MI_PLAYERMAP_INSTALL_ERROR_TABLE_DOROPPED,
                            $tableName
                        )
                    );
                }
    ...
    **/
    ...
    {
        Playermap_InstallUtils::uninstallAllOfModuleTemplates($this->_mXoopsModule,$this->mLog,false);
    }
    ...
    **/
    private function _uninstallBlocks()
    {
        ...
        if(!$tplHandler->deleteAll($cri))
        {
            $this->mLog->addError(
                XCube_Utils::formatString(
                    _MI_PLAYERMAP_INSTALL_ERROR_BLOCK_TPL_DELETED,
                    $tplHandler->db->error()
                )
            );
        }
    ...
    **/
    ...
    {
        Playermap_InstallUtils::uninstallAllOfConfigs($this->_mXoopsModule,$this->mLog);
    }
    ...
    **/
    private function _processReport()
    {
        if(!$this->mLog->hasError())
        {
            $this->mLog->add(
                XCube_Utils::formatString(
                    _MI_PLAYERMAP_INSTALL_MSG_MODULE_UNINSTALLED,
        ...
        else if(is_object($this->_mXoopsModule))
        {
            $this->mLog->addError(
                XCube_Utils::formatString(
                    _MI_PLAYERMAP_INSTALL_ERROR_MODULE_UNINSTALLED,
                    $this->_mXoopsModule->get('name')
                )
            );
        }
